...
import "./InterestRateModel2.sol";
import "./ReentrancyGuard.sol";
import "./CToken.sol";

contract CToken2 is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    /**
    ...
    /**
     * @notice reserver for this contract
     */
     ...
     */
    InterestRateModel2 public interestRateModel;
     ...
     */
    event NewMarketInterestRateModel(InterestRateModel2 oldInterestRateModel, InterestRateModel2 newInterestRateModel);
    ...
    constructor(ComptrollerInterface comptroller_,
                InterestRateModel2 interestRateModel_,
    ...
    function _reduceReservesFresh(uint reduceAmount) internal returns (uint) {
        ...
        // Check caller is admin
        if (msg.sender != admin && msg.sender != reserver) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setInterestRateModel(InterestRateModel2 newInterestRateModel) public returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setInterestRateModelFresh(InterestRateModel2 newInterestRateModel) internal returns (uint) {

        // Used to store old model for use in the event that is emitted on success
        InterestRateModel2 oldInterestRateModel;
